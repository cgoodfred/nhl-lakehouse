...
- **All images auto-pin to SHA via `chore/pin-<component>-image` PRs** opened by each `build-*-image.yml`. Merging a viz pin PR is a real deploy (`infra/viz.tf` → `tofu apply` in `deploy.yml`); ingest and spark pin PRs update git only — operator still `kubectl apply`s spark manifests / re-invokes the ingest Job for the new SHA to hit the cluster. `spark/k8s/**` is excluded from the spark build trigger to prevent pin-PR → build loops.
