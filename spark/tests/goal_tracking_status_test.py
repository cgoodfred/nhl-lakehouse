...
def _sequences(spark, rows):
    return spark.createDataFrame(rows, _SEQUENCES_SCHEMA)
...
def _sequence_for(event_id: int, frame_count: int = 140):
    return [(_SEASON, _GAME, event_id, frame_count)]
...
def _build_inputs(spark, plays_rows, attempts_rows, frames_rows, sequences_rows=None):
    return (
        _plays(spark, plays_rows),
        _attempts(spark, attempts_rows),
        _frames(spark, frames_rows),
        _sequences(spark, sequences_rows or []),
    )
...
def test_available_when_success_frames_and_gold_sequence_present(spark):
    plays, attempts, frames, sequences = _build_inputs(
        spark,
        ...
        sequences_rows=_sequence_for(100, 140),
    )
    rows = _by_event(transform_goal_tracking_status(plays, attempts, frames, sequences))
...
def test_pending_gold_sequence_rebuild_when_silver_frames_exist_but_gold_missing(spark):
    ...
    rows = _by_event(transform_goal_tracking_status(plays, attempts, frames, sequences))
    assert rows[100].tracking_status == "pending_gold_sequence_rebuild"
    assert rows[100].frame_count == 140
    assert rows[100].fetch_status == "success"
...
def test_one_row_per_goal_no_join_fanout(spark):
    ...
    plays, attempts, frames, sequences = _build_inputs(
        ...
        sequences_rows=_sequence_for(100, 140) + _sequence_for(200, 120),
    )
    out_rows = transform_goal_tracking_status(plays, attempts, frames, sequences).collect()
...
def test_all_status_branches_in_one_run(spark):
    ...
    assert {evid: row.tracking_status for evid, row in out.items()} == {
        100: "no_url",
        200: "available",
        300: "pending_gold_sequence_rebuild",
        400: "pending_silver_rebuild",
        500: "not_tracked",
        600: "fetch_failed",
        700: "not_attempted",
    }
